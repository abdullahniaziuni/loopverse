...
export const MyBookings: React.FC = () => {
  const [selectedTab, setSelectedTab] = useState<'upcoming' | 'pending'>('upcoming');
  ...
  const confirmCancellation = () => {
    if (cancellingSession) {
      // Mock cancellation
      showSuccess('Session cancelled successfully. You will receive a confirmation email.');
  ...
  const handleJoinSession = (session: Session) => {
    // Mock join session
    showSuccess('Joining session... You will be redirected to the video call.');
  };
  ...
  const currentSessions = selectedTab === 'upcoming' ? upcomingSessions : pendingSessions;
          ...
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setSelectedTab(tab.id as 'upcoming' | 'pending')}
            ...
            <div className="mb-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{cancellingSession.mentorName}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Date & Time:</span>
                  <span className="font-medium">
                    {formatDate(cancellingSession.date)} at {formatTime(cancellingSession.startTime)}
                  </span>
                </div>
              </div>
              <p className="text-sm text-gray-600 mt-4">
                <strong>Note:</strong> Cancellations made less than 24 hours before the session may be subject to a cancellation fee.
              </p>
            </div>
...
const SessionCard: React.FC<SessionCardProps> = ({ session, onCancel, onJoin }) => {
