...
const Session = require("./backend/LoopVerse/Models/session");
const Learner = require("./backend/LoopVerse/Models/learner");
const Mentor = require("./backend/LoopVerse/Models/mentor");
...
async function checkBookingStatus() {
  try {
    ...
    sessions.forEach((session, index) => {
      ...
      console.log(
        `   Learner: ${session.learnerId?.firstName} ${session.learnerId?.lastName} (${session.learnerId?.email})`
      );
      console.log(
        `   Mentor: ${session.mentorId?.firstName} ${session.mentorId?.lastName} (${session.mentorId?.email})`
      );
    ...
    if (learner) {
      ...
      learnerSessions.forEach((session, index) => {
        console.log(
          `${index + 1}. ${session.title} - Status: ${session.status}`
        );
        console.log(
