...
async function seedMentors() {
  try {
    ...
    const mentors = [
      {
        ...
        certifications: ["AWS Certified Solutions Architect", "React Certified Developer"],
        specializations: ["Web Development", "Career Guidance", "Technical Leadership"],
    ...
    const result = await mongoose.connection.db.collection("mentors").insertMany(mentors);
    ...
    mentors.forEach((mentor, index) => {
      console.log(`${index + 1}. ${mentor.name} - ${mentor.skills.join(", ")} - $${mentor.hourlyRate}/hr`);
    });
